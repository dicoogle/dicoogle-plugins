 ...
 */
public class SeriesJettyWebService extends HttpServlet implements PlatformCommunicatorInterface {
    private static final Logger logger = LoggerFactory.getLogger(SeriesJettyWebService.class);
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse response)
        ...
        try {
            ...
            Task<Iterable<SearchResult>> result = this.platform.query("lucene", "StudyInstanceUID:"+req.getParameter("StudyInstanceUID"), extraFields);
            rr = result.get();
        ...
        if (rr != null) {
            for (SearchResult sr : rr) {
                ...
                if (keys.add((String)sr.get("SeriesInstanceUID"))) {
// Read Dicom from URI
        ...
        // if modality == ANN, SEG or OT, then return emtpy json array
        if (modality != null && (modality.equals("ANN") || modality.equals("SEG") || modality.equals("OT"))) {
